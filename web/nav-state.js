...
function trapSidebarTab(e){if(e.key!=='Tab'||innerWidth>=980||!document.body.classList.contains('atlas-sidebar-open'))return;const side=$('#atlasSidebar');if(!side)return;const focusable=$$('button:not([disabled]),a[href],input:not([disabled]),textarea:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])',side).filter(x=>!x.hidden&&x.offsetParent!==null);if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1],active=document.activeElement;if(e.shiftKey&&(active===first||!side.contains(active))){e.preventDefault();last.focus();return}if(!e.shiftKey&&(active===last||!side.contains(active))){e.preventDefault();first.focus()}}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&innerWidth<980&&document.body.classList.contains('atlas-sidebar-open')){collapseSidebar();$('#atlasSidebarToggle')?.focus();return}trapSidebarTab(e)});
